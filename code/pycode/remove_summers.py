...
cz_mask = file_id.variables["CZ_mask"][:].astype(bool)
file_id.close()
...
# -- Save summer-only anomaly as NetCDF --
out_path = '/home/jamesash/koa_scratch/chl_anomaly_summer_20260328.nc'
...
anom_var.long_name = 'Chlorophyll anomaly, summer only (Jun-Oct)'

mask_var = out.createVariable('CZ_mask', 'i1', ('latitude', 'longitude'), zlib=True)
mask_var[:] = cz_mask.astype(np.int8)
mask_var.long_name = 'Contiguous zone mask (1 = inside CZ, 0 = open ocean)'

out.description = 'CHL anomaly, cropped, CZ masked, summer months only (Jun-Oct)'
out.source_file = 'chl_anomaly_cropped_masked_20260328.nc'
